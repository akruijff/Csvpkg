...
public class ClearCell extends AbstractCommand {
    @Override
    ...
    protected void doHelp(String[] args) {
        System.err.println("""
                clear <condition column> <modification column> <regular expression> [<flags>]
    ...
    protected Sheet doExecute(Sheet sheet, String[] args) {
        ...
        return clearCell(sheet, args[1], args[2], pattern);
    }

    public static Sheet clearCell(Sheet sheet, String conditionColumn, String modificationColumn, Pattern pattern) {
        Cell[] cells = new Mapper(sheet, conditionColumn, modificationColumn, pattern)
                .mapCells(cell -> new Cell(cell.header(), cell.lineNumber(), ""));
